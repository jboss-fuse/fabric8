...
public class JoinTest {
	...
	public void testJoin() throws Exception {
        ...
        try {
            ...
            System.err.println(CommandSupport.executeCommand("admin:create -o '-server -Xmx1536M -Dcom.sun.management.jmxremote -XX:+UnlockDiagnosticVMOptions -XX:+UnsyncloadClass' --featureURL mvn:io.fabric8/fabric8-karaf/" + version + "/xml/features --feature fabric-git --feature fabric-agent --feature fabric-boot-commands smoke.child_d"));

            try {
                System.err.println(CommandSupport.executeCommand("admin:start smoke.child_d"));
                ProvisionSupport.instanceStarted(Arrays.asList("smoke.child_d"), ProvisionSupport.PROVISION_TIMEOUT);
                ...
                for (int i = 0; i < 10 && !response.contains("true"); i++) {
                    response = CommandSupport.executeCommand("ssh:ssh -l karaf -P karaf -p " + adminService.getInstance("smoke.child_d").getSshPort() + " localhost " + WAIT_FOR_JOIN_SERVICE);
                    Thread.sleep(1000);
                }

                System.err.println(CommandSupport.executeCommand("ssh:ssh -l karaf -P karaf -p " + adminService.getInstance("smoke.child_d").getSshPort() + " localhost " + joinCommand));
                ProvisionSupport.containersExist(Arrays.asList("smoke.child_d"), ProvisionSupport.PROVISION_TIMEOUT);
                Container childD = fabricService.getContainer("smoke.child_d");
                ...
                System.err.println(CommandSupport.executeCommand("admin:stop smoke.child_d"));
            }
        } finally {
            fabricProxy.close();
        }
