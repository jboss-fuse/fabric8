 ...
 */
package io.fabric8.insight.elasticsearch2;
...
public class Elasticsearch implements ElasticsearchMBean {
    ...
    @Reference(name = "node", policy = ReferencePolicy.DYNAMIC, referenceInterface = io.fabric8.insight.elasticsearch2.impl.ElasticsearchNode.class, cardinality = ReferenceCardinality.OPTIONAL_MULTIPLE)
    private final Map<String, Set<Node>> nodesClusterMap = new ConcurrentHashMap<String, Set<Node>>();

    @Activate
    void activate(Map<String, ?> configuration) throws Exception {
        JMXUtils.registerMBean(this, mbeanServer, new ObjectName("io.fabric8.insight:type=Elasticsearch2"));
    }

    @Deactivate
    void deactivate(Map<String, ?> configuration) throws Exception {
        JMXUtils.unregisterMBean(mbeanServer, new ObjectName("io.fabric8.insight:type=Elasticsearch2"));
    }
