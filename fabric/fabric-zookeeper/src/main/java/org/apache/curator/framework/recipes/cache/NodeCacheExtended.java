 ...
 */
public class NodeCacheExtended implements Closeable
{
    ...
    private final ListenerContainer<NodeCacheExtendedListener> listeners = new ListenerContainer<NodeCacheExtendedListener>();
     ...
     */
    public NodeCacheExtended(CuratorFramework client, String path)
    {
     ...
     */
    public NodeCacheExtended(CuratorFramework client, String path, boolean dataIsCompressed)
    {
        this.client = client;
        this.path = path;
    ...
    {
        final ChildData   previousData = data.getAndSet(newData);
        if ( !Objects.equal(previousData, newData) )
        {
            listeners.forEach
            (
                new Function<NodeCacheExtendedListener, Void>()
                {
                    @Override
                    public Void apply(NodeCacheExtendedListener listener)
                    {
                        try
                        {
                            listener.nodeChanged(previousData, newData);
                        }
                        catch ( Exception e )
                        {
