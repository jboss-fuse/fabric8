...
public class SaslQuorumAuthServer implements QuorumAuthServer {
    ...
    private Map<String, String> peerCredentials;

    public SaslQuorumAuthServer(boolean quorumRequireSasl, Set<String> authzHosts, Map<String, String> peerCredentials)
    ...
    public void authenticate(Socket sock, DataInputStream din)
        ...
        try {
            ...
            byte[] challenge = null;
            // for DIGEST-MD5 we don't need any server-side subject - or rather no principals inside
            // otherwise we'd get GSSAPI SASL mechanism
            // ZOOKEEPER-3138: we're using thread/connection specific JAAS CallbackHandler, not peer-specific one
            SaslQuorumServerCallbackHandler callbackHandler = new SaslQuorumServerCallbackHandler(peerCredentials, authzHosts);
            ss = SecurityUtils.createSaslServer(new Subject(),
                    QuorumAuth.QUORUM_SERVER_PROTOCOL_NAME,
                    QuorumAuth.QUORUM_SERVER_SASL_DIGEST, callbackHandler,
                    LOG);
            while (!ss.isComplete()) {
