...
public class FabricLoadBalancerFeature extends AbstractFeature implements BusLifeCycleListener {
    ...
    public synchronized Group getGroup() throws Exception {
         if (group == null) {
             group = new ZooKeeperGroup<CxfNodeState>("Fabric CXF Load Balancer", getCurator(), zkRoot + fabricPath, CxfNodeState.class, new NamedThreadFactory("zkgroup-lb-feature"));
             group.start();
         }
        return group;
    }
